...
#
# Distroless multi-stage build for the Rust Senzing RabbitMQ consumer.
#
#   docker build -t brian/sz_rabbit_consumer_rust .                     # both backends
...
LABEL org.opencontainers.image.title="sz_rabbit_consumer_rust" \
      org.opencontainers.image.description="Scalable Senzing RabbitMQ consumer (Rust)" \
      org.opencontainers.image.licenses="Apache-2.0"

COPY --from=builder /app/target/release/sz_rabbit_consumer /usr/local/bin/sz_rabbit_consumer

ENTRYPOINT ["/usr/local/bin/sz_rabbit_consumer"]
